...
createBarrelRollup('services');
createBarrelRollup('mirror');

function createBarrelRollup(subDir) {
  const projDir = path.resolve(path.dirname(url.fileURLToPath(import.meta.url)), '..');
  const baseSrcDir = path.join(projDir, 'src', subDir);
  const allSources = fs.readdirSync(path.join(baseSrcDir, subDir)).filter(f => f.endsWith('.ts')).map(f => path.join(baseSrcDir, subDir, f));
  const protoSymbols = {};
  const protoExcludes = [
    'DeepPartial'
  ];
